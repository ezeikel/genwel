 ...
 *
 * Provides consistent logging across the application with integration to:
...
class Logger {
    ...
    try {
      ...
      switch (entry.level) {
        ...
        case 'warn':
          ...
          break;
        case 'error':
          // Errors definitely go to Sentry
          if (entry.error) {
  ...
  aiError(message: string, error?: Error, context?: LogContext) {
    this.error(
      message,
      {
  ...
  sanityError(message: string, error?: Error, context?: LogContext) {
    this.error(
      message,
      {
        error_type: 'sanity',
  ...
  // Blog generation logging
  logBlogGeneration(step: string, context?: LogContext) {
    this.info(`Blog generation: ${step}`, {
      action_type: 'blog_generation',
      ...context,
    });
  }
}
